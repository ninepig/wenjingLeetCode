...
/**
 * Created by yamengwenjing on 2016-11-26.
 */
public class AllTreeProblemSet {
    /**
     *  use recersive way to get TotAL number of tree node
     * @param root
     * @return
     * 1 如果二叉树为空，节点数为0
     * 2如果二叉树不为空，二叉树节点数 =左子树个数+右子树个数+1
     */
   public int getNodeNumRec(TreeNode root){
     ...
     * @param root
     * @return
     * like level order travelsal,一层遍历下去，将所有的都加进去
     */
   public int getNodeNumIter(TreeNode root){
       if(root ==null){
           return  0;
       }
        int count =1;
     ...
     * @param root
     * @return
     * 获取最大深度， 如果节点为空 返回0， 如果不为空，返回左右两侧比较深的+1
     */
    public int getDepthRc(TreeNode root){
        if(root ==null){
            return 0;
        }
        int left = getDepthRc(root.left);
        int right = getDepthRc(root.right);
     ...
     * @param root
     * @return
     * 获取最大深度，层序遍历走下去，同时用一个conter 记录深度即可
     */
    public int getDepthIter(TreeNode root){
        ...
        int depth =0;
        int currentLevelNodeNumber =0;
        int nextLevelNodeNumber = 0;
        Queue<TreeNode> thisQueue = new LinkedList<>();
        thisQueue.offer(root);
        currentLevelNodeNumber++;
     ...
     * @param root
     */
    public void preOrderTravelRec(TreeNode root){
        if(root ==null){
            return ;
        }
        System.out.println(root.val);
        preOrderTravelRec(root.left);
        preOrderTravelRec(root.right);
     ...
     * * @param root
     */
    public void preOderTravelIter(TreeNode root){
        if(root==null){
            return;
        }
        ...
        while (!stack.isEmpty()){
            ...
            //key point , you should push right child node, then left. in this way ， it will print the left then right
            if(cur.right !=null){
     ...
     * @param root
     */
    public void inOrderTravlRec(TreeNode root){
        if(root == null){
            return;
        }
        inOrderTravlRec(root.left);
        System.out.println(root.val);
        inOrderTravlRec(root.right);
    }

    /**
     * 用一个栈来做，关键点，全部push左边的，然后再pop ，再打印右边的。因为左边的全部被打印出来了，一层一层弹出来，中间，右边的就在左边之后
     * @param root
     */
    public void inOrderTravlIter(TreeNode root){
        if(root == null){
            return;
        }
        ...
        TreeNode cur  = root;
        while(true){
            //push all the left child node in stack
            while (cur!=null){
                thisStack.push(cur);
                cur = cur.left;
            }
            ...
            //there is no left child, so printout top of stack;
            cur = thisStack.pop();
            System.out.print(cur.val);
            cur = cur.right;
            /*
            一直把最左边的printout 然后中间的，然后再右侧的，模拟的很对，仔细想想。
             */
        }
    }

    /**
     * rec way to do postOrderTravel
     * @param root
     */
    public void postOrderTravelReC(TreeNode root){
        if(root == null){
            return;
        }
...
/**
 * 和inOrder的很像，但是是利用多了一个stack ，递归的加入第一个栈，但同时把栈顶元素翻转到第二个栈中，最后再输出第二个栈，所以是先左再右。要仔细想想过程。
 */
    public void postOderTravelInter(TreeNode root){
        if(root ==null ){
            return;
        }
    ...
    public void levelTraverSal(TreeNode root){
       ...
       Queue<TreeNode> thisQueue = new LinkedList<>();
        thisQueue.offer(root);
        while(!thisQueue.isEmpty()){
            TreeNode cur = thisQueue.poll();
            System.out.println(cur.val);
            if(cur.left!=null){
                thisQueue.offer(cur.left);
            }else  if(cur.right!=null){
                thisQueue.offer(cur.right);
            }
        }
    }

    /**
     * 用递归做层序遍历， 一般用一个大的arraylist ，里面是每一层的arraylist，和层序遍历差不多，这个题有点牛逼。
     */
